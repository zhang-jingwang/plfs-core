...
plfs_error_t
PosixIOSHandle::Size(off_t *ret_offset) {
    POSIX_IO_ENTER(this->bpath.c_str());
    off_t rv;
...
plfs_error_t
PosixIOStore::Open(const char *bpath, int flags, mode_t mode, IOSHandle **ret_hand) {
...
plfs_error_t
PosixIOStore::Opendir(const char *bpath, IOSDirHandle **ret_dirh) {
    POSIX_IO_ENTER(bpath);
    DIR *dp;
