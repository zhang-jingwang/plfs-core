...
FuseIOSHandle::Close() {
    ...
    int rv;
    rv = this->fuse_ops->release("PathUnused", &this->fuse_fi);
...
FuseIOSHandle::Fstat(struct stat* buf) {
    FUSE_IO_ENTER(this->bpath.c_str());
    int rv;
    rv = this->fuse_ops->fgetattr("PathUnused", buf, &this->fuse_fi);
    FUSE_IO_EXIT(this->bpath.c_str(),rv);
    return(get_err(rv));
}
...
FuseIOSHandle::Fsync() {
    FUSE_IO_ENTER(this->bpath.c_str());
    int rv;
    rv = this->fuse_ops->fsync("PathUnused", 0, &this->fuse_fi);
    FUSE_IO_EXIT(this->bpath.c_str(),rv);
    return(get_err(rv));
}
...
FuseIOSHandle::Ftruncate(off_t length) {
    FUSE_IO_ENTER(this->bpath.c_str());
    int rv;
    rv = this->fuse_ops->ftruncate("PathUnused", length, &this->fuse_fi);
    FUSE_IO_EXIT(this->bpath.c_str(),rv);
    return(get_err(rv));
}
...
FuseIOSHandle::Pread(void* buf, size_t count, off_t offset) {
    FUSE_IO_ENTER(this->bpath.c_str());
    ssize_t rv;
    rv = this->fuse_ops->read("PathUnused", (char *)buf, count, offset,
                              &this->fuse_fi);
    FUSE_IO_EXIT(this->bpath.c_str(),rv);
    return(get_err(rv));
}
...
FuseIOSHandle::Pwrite(const void* buf, size_t count, off_t offset) {
    FUSE_IO_ENTER(this->bpath.c_str());
    ssize_t rv;
    rv = this->fuse_ops->write("PathUnused", (const char *)buf, count,
                               offset, &this->fuse_fi);
    FUSE_IO_EXIT(this->bpath.c_str(),rv);
    return(get_err(rv));
}
...
FuseIOSDirHandle::loadDentries() {
    ...
    ret = this->fuse_ops->readdir(this->bpath.c_str(), this,
                                  ...
                                  &fi);
    this->fuse_ops->releasedir("PathUnused", &fi);
    return ret;
}
...
FuseIOStore::Lchown(const char *path, uid_t owner, gid_t group) {
    FUSE_IO_ENTER(path);
    int rv = 0;
    /* TODO: How to implement lchown based on FUSE? */
    FUSE_IO_EXIT(path,rv);
    return(get_err(rv));
}
...
FuseIOStore::Open(const char *bpath, int flags, mode_t mode, int &ret) {
    ...
    if (hand == NULL) {
        ret = -ENOMEM;
        this->fuse_ops->release("PathUnused", &fi);
    }
    FUSE_IO_EXIT(bpath,0);
    return(hand);
}
